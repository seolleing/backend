...
public class LikeService {
    ...
    @Transactional
    public void saveLikeNovel(Long novelId, Long userId) {
        if (likeNovelRepository.existsByNovel_NovelIdAndUser_UserId(novelId, userId)) {
            throw new AlreadyDoingException(
                    ExceptionEnum.ALREADYDOING.ordinal(), "이미 좋아요를 누르셨습니다.", HttpStatus.FORBIDDEN);
        } else {
    ...
    @Transactional
    public void saveLikeComment(Long id, Long userId) {
        if (likeCommentRepository.existsByComment_IdAndUser_UserId(id, userId)) {
            throw new AlreadyDoingException(
                    ExceptionEnum.ALREADYDOING.ordinal(), "이미 좋아요를 누르셨습니다.", HttpStatus.FORBIDDEN);
        } else {
            UserEntity userEntity = userRepository.findById(userId)
